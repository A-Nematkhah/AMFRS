...
- **Accepted** — architecture decision locked for subsequent phases (may still need empirical weights)

### Phase 0 — Doc / code alignment (done)

| Item | Canonical name in this repo |
|------|-----------------------------|
| Env tree | `amfrs_env/` |
| Pipeline | `AMFRSPipeline`, `AMFRSRunConfig`, `AMFRSArtifacts` |
| CLI | `scripts/run_amfrs.py`, `scripts/run_amfrs_paper_scale.py` |
| Post-hoc | `eval_amfrs_checkpoint.py`, `visualize_amfrs.py`, `plot_amfrs_run.py`, `report.py` |
| CI | `.github/workflows/amfrs-ci.yml` |
| Docs | `README.md`, `amfrs_env/README_AMFRS.md` |

EvoNav remains the **baseline paper / Algorithm 1 substance** we replicate and cite; AMFRS is the **project name** for thesis contributions built on top.

**Locked decisions (Phase 0):** D-1 = **A**, D-4 = **B**, D-5 = **B**, D-6 = **B**.  
**Still pending:** D-2 (empirical). **D-3 numeric weights:** Accepted v0 in `SelectionWeights` (T7; Needs Validation).

---

## 1. Executive Summary

The project today is a CrowdNav++ fork plus an EvoNav Algorithm 1 pipeline (rebranded under AMFRS): LLM-proposed reward functions are sandboxed, ranked analytically on a fixed trajectory dataset (**Stage I**), refined under short proxy RL (**Stage II**, typically A2C), then trained/evaluated at near-full budget with PPO (**Stage III**), optionally with a human-count sweep. Entry is primarily `scripts/run_amfrs.py` → `AMFRSPipeline.run()`, with paper budgets via `configs/paper_scale.yaml` / `run_amfrs_paper_scale.py`.
...
| Entry | Role |
|-------|------|
| `scripts/run_amfrs.py` | Algorithm 1 end-to-end |
| `scripts/run_amfrs_paper_scale.py` | Multi-seed paper budgets; loads `configs/paper_scale.yaml` |
...
| `scripts/eval_amfrs_checkpoint.py`, `visualize_amfrs.py`, `report.py`, `plot_amfrs_run.py` | Post-hoc analysis |

### 2.2 Pipeline / orchestrator

- `crowd_nav/reward_search/pipeline.py` — `AMFRSPipeline` / `AMFRSRunConfig` / `AMFRSArtifacts`  
...
- Scalar: `SR - CR - 0.5·TR - 0.01·ITR + 0.2·SD` via `selection.SelectionWeights` (T7 / D-3 v0)

### 2.10 Experiment configuration

- `AMFRSRunConfig`, CLI flags, `regime.py`, `presets.py` / `paper_scale.yaml`  
...
```text
Current:
User/CLI → run_amfrs / run_amfrs_paper_scale
        → AMFRSPipeline
...
1. Top-k finalists only — **done** (Phase 3 / T8)  
2. Mandatory accept/reject (stricter than II) — **done** (Phase 3 / T9; skip last round when G3>1)  
3. Multi-seed reporting (paper-scale already multi-seed at pipeline level)  
4. H-aware / constrained final selection — **done** (Phase 3 / T11; mean/worst-H blend; ITR/Pareto still later)  
...
- **Status:** **Accepted (Phase 0)** — Option A; not yet implemented in code
...
- **Status:** **Decision Pending** (empirical; default remains A2C until measured)
...
- **Status:** **Accepted (Phase 0 + T7)** — Option B with v0 weights in `selection.SelectionWeights` (`w_tr=0.5`, `w_itr=0.01` on percent ITR, `w_sd=0.2`); still **Needs Validation** empirically; Option C (Pareto) remains Phase 5.
...
- **Status:** **Accepted (Phase 0)** — Option B; not yet implemented in code

**Decision D-5 — When to introduce AMFRS**

- **Question:** Add archive/Pareto now or after metric stabilization?  
- **Option A:** Now.  
- **Option B:** After P0 Score1+proxy gates (Phases 1–3 of this roadmap).  
- **Recommended:** **B**  
...
- **Status:** **Accepted (Phase 0)** — Option B
...
- **Status:** **Accepted (Phase 0)** — Option B; surrogate remains **Needs Validation** if revisited
...
### Phase 0 — Doc / code name alignment (done 2026-09-10)

| Task | What | Result |
|------|------|--------|
| P0-1 | Align this document with `amfrs_env` / `AMFRS*` / `run_amfrs*` | Paths and class names match the tree |
| P0-2 | Lock D-1=A, D-4=B, D-5=B, D-6=B; D-3 direction=B | Decisions section status updated |
| P0-3 | No algorithm logic changes | Pipeline behavior unchanged |

Next actionable phase: **Phase 5** (AMFRS modules) after empirical validation of higher-K3 SR, or resolve D-2.

### Phase 1 — Trustworthy Stage I (P0) — **done 2026-09-10**

| Task | What | Where | Why | Dependency | Expected result | Validation |
|------|------|-------|-----|------------|-----------------|------------|
| T1 | Pad masking + finite/variance guards | `scoring.py`, tests | Fix Score1 bias | None | Stable Score1 | Unit tests unequal lengths / constant r — **done** |
| T2 | Holdout split + manifest fields | `dataset`/`scoring`/`pipeline` | Detect overfit | T1 | train/holdout scores logged | Fixture dataset split — **done** |
| T3 | Smoke/paper Score1 hard labeling | CLI/reporting | Avoid false claims | None | Clear mode in artifacts | Test manifest — **done** |

**Code surface:** `Score1Options`, `score1_report`, `split_stage1_dataset`, `score1_mode_artifact`; `AMFRSRunConfig.stage1_holdout_fraction`; manifest key `score1`.

### Phase 2 — Stable Stage II gate (P0–P1) — **done 2026-09-10**

| Task | What | Where | Why | Dependency | Expected result | Validation |
|------|------|-------|-----|------------|-----------------|------------|
| T4 | Multi-seed proxy aggregate | `stage2.py` | Rank stability | Phase1 optional | mean±std in metadata | Repeatability study — **done** (`n_eval_seeds`, `aggregate_proxy_metrics`) |
| T5 | Accept/reject refine | `stage2.py` | Stop regressions | T4 helpful | Only improving codes kept | Synthetic refine tests — **done** |
| T6 | Failure-mode diagnostics v0 | `diagnostics.py` → D.3 | Better D.3 | — | Structured critique in refine prompts | `test_diagnostics` — **done** |
| T7 | Selection weights v0 (incl. ITR/SD) | `selection.py` | Social signal | D-3 weights | Shared scalar II/III | `test_selection` — **done** (v0; Needs Validation) |

**Code surface:** `Stage2Config.n_eval_seeds` / `accept_reject_*`; `train_eval_multiseed`; `_maybe_accept_refine`; pipeline `stage2_n_eval_seeds` (fast→1).

### Phase 1.5 / 2.1 — Hotfix (done 2026-09-10)

| Fix | What | Where |
|-----|------|-------|
| H1 | Exclude padded frames from Spearman (not only freeze cumulative) | `scoring._scenario_frame_correlations` |
| H2 | Accept/reject compares parent vs proposed under the **same** verify budget | `stage2._maybe_accept_refine` |
| H3 | Per-seed output dirs: `r{rr}_{id}_seed{seed}` | `stage2._stage2_train_argv` |

### Phase 3 — Stage III as finalist eval (P0–P1) — **done 2026-09-10**

| Task | What | Where | Why | Dependency | Expected result | Validation |
|------|------|-------|-----|------------|-----------------|------------|
| T8 | Top-k admission | `pipeline.py` | Save K3 | T4–T5 | Smaller III pop | Iso-quality GPU-hours — **done** (`stage3_max_finalists`, `select_top_k_finalists`) |
| T9 | Strict refine policy | `stage3.py` | Costly mistakes | T5 | Conservative III | No elite refine harm — **done** (fair-budget accept/reject + skip refine on last round when G3>1) |
| T10 | Artifact genome clarity | `reporting.py`, pipeline | Reproducibility | None | evaluated vs refined fields | Unit tests — **done** |
| T11 | H-aware final select | `stage3.py`, `selection.py` | Generalization | T8 | Winner considers H | Worst-H metric — **done** (`h_profile_scalar`, top-2 H-sweep) |

**Code surface:** `AMFRSRunConfig.stage3_max_finalists` / `stage3_accept_reject_*` / `stage3_h_sweep_max_finalists`; `Stage3Config` gates; `selection.select_top_k_finalists` / `h_profile_scalar` / `attach_h_profile`; artifacts `stage3_finalists.json`, `evaluated_genome` / `proposed_refined_genome`.

### Phase 4 — Maintainability + alignment (P1) — **done 2026-09-12**

| Task | What | Where | Why | Dependency | Expected result | Validation |
|------|------|-------|-----|------------|-----------------|------------|
| T12 | Shared train/refine harness | `refine_harness.py` ← stage2/3 | One accept/reject + repair path | Characterization tests | Less duplication | Existing + `test_phase4` — **done** |
| T13 | II↔III rank logging | `reporting.py`, `pipeline.py` | Measure mismatch | T4,T8 | Calibration table | `stage2_stage3_calibration.json` — **done** |
| T14 | Stage I diagnostics→mutation | `scoring.py`, `evolver.py` | Better I search | T1–T2 | Weak-scenario prompts | Mutation weakness contains diagnostics — **done** |

**Code surface:** `refine_harness.decide_accept_reject` / metadata builders / `repair_invalid_code`; `reporting.stage2_stage3_calibration`; `Score1Report.per_scenario` + `format_score1_diagnostics` + `make_score1_report_fn`; evolver stores `metadata["score1_report"]`.
...
0. **Phase 0 (done):** Align docs/names with `amfrs_env` + lock D-1/D-4/D-5/D-6 (and D-3 direction).  
1. **Phase 1 (done):** Make Stage I Score1 scientifically trustworthy (mask pads, anti-exploit, holdout).  
2. **Phase 2 (done):** Make Stage II a reliable gate (multi-seed metrics + accept/reject refine). **T6/T7 done** (failure-mode D.3 + ITR/SD weights v0).  
3. **Phase 3 (done):** Convert Stage III into a **finalist PPO tournament** with H-aware selection and strict refine policy.  
4. **Phase 4 (done):** Shared accept/reject harness, II↔III calibration artifact, Score1 diagnostics into mutation prompts.  
5. **Only then** add AMFRS (Pareto/archive/diversity) on stable candidate/metrics interfaces — do not expand G3/K3 or build surrogates as a substitute for those gates.
