#!/usr/bin/env python
"""
Entry point for AMFRS2 (innovation track).

Does not invoke the EvoNav-faithful ``run_amfrs.py`` / ``AMFRSPipeline``.

Examples (from ``amfrs_env/``)::

    python scripts/run_amfrs2.py --fast --output-dir results/amfrs2_fast

    # Real trainers without GST (obs = none):
    python scripts/run_amfrs2.py --allow-seed-llm --predict-method none \\
        --score1 smoke --device cpu --output-dir results/amfrs2_real_none

    # Real trainers with GST (after scripts/fetch_gst_weights.py):
    python scripts/run_amfrs2.py --allow-seed-llm --predict-method inferred \\
        --score1 smoke --device cuda --output-dir results/amfrs2_real_gst
"""
...
def main() -> int:
    ...
    from crowd_nav.reward_search.amfrs.assets import check_amfrs2_assets

    parser = argparse.ArgumentParser(description="AMFRS2 end-to-end (innovation track)")
    parser.add_argument("--output-dir", type=str, default="results/amfrs2_run")
    ...
    parser.add_argument(
        ...
        help="Stage I / F0 scorer (default dataset; smoke if --fast)",
    )
    ...
    if (
        ...
        print(
            "Refusing non-fast AMFRS2 with --llm seed. "
    ...
    print(
        f"[amfrs2] done | accepted={len(artifacts.accepted_candidates)} "
