...
Locks in:
  1) AMFRS Stage III best-ever (R0 mut_0060_v2) vs last-round (R1) policies
...
_spec = importlib.util.spec_from_file_location(
    "eval_amfrs_checkpoint",
    os.path.join(_ROOT, "scripts", "eval_amfrs_checkpoint.py"),
)
...
def eval_amfrs_multi_seed(
    *,
    ...
    rows = []
    for seed in seeds:
        print(f"\n=== AMFRS {candidate_id} r{round_index:02d} seed={seed} E={episodes} ===")
        row = run_eval(
            run_dir=run_dir,
        ...
        rows.append(row)
    return {
        "method": f"AMFRS_{candidate_id}_r{round_index:02d}",
        "candidate_id": candidate_id,
        "round_index": round_index,
...
def main() -> int:
    ...
    # 1) Best-ever R0
    report["methods"]["AMFRS_best_ever_r00"] = eval_amfrs_multi_seed(
        run_dir=run_dir,
    ...
    report["methods"]["AMFRS_last_round_r01"] = eval_amfrs_multi_seed(
        run_dir=run_dir,
        candidate_id="mut_0060_v2",
        round_index=1,
        episodes=int(args.episodes),
