#!/usr/bin/env python
"""
Eval-only re-evaluation of an AMFRS Stage III checkpoint (no training).
...
Examples::
    ...
    python scripts/eval_amfrs_checkpoint.py \\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Eval-only re-evaluation of an AMFRS Stage III checkpoint"
    )
    parser.add_argument(
        "--run-dir",
        required=True,
        help="AMFRS result directory (contains config.json, stage3_train/...)",
    )
    parser.add_argument(
        "--candidate-id",
        default=None,
