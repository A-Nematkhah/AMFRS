#!/usr/bin/env python
"""
Reproduce baseline paper Table 1 (baselines) and Table 2 (stage ablation) structures.
...
Table 1 (method comparison, with / without human randomization):
  SF, ORCA, DS-RNN, CrowdNav++ (LegacyReward-trained), AMFRS final
...
Examples::
    ...
    python scripts/report.py --run-dir results/amfrs_fast --fast \\
        --output results/amfrs_fast/report.json --skip-dsrnn
...
def build_table1(
    *,
    ...
    skip_dsrnn: bool,
    skip_missing: bool,
    include_amfrs_train: bool,
    amfrs_train_steps: int,
    use_stub: bool,
    work_dir: str,
) -> Dict[str, Any]:
    ...
    # AMFRS final from run_dir
    if run_dir and os.path.isfile(os.path.join(run_dir, "final_candidate.json")):
        cand = load_candidate_dict(
            _load_json(os.path.join(run_dir, "final_candidate.json"))
        )
        for bucket, rand in (("no_rand", False), ("rand", True)):
            logging.info("Table 1 AMFRS (%s)", bucket)
            if use_stub or not include_amfrs_train:
                ...
                ev = summarize_episodes(
                    all_eps, method="AMFRS", randomize=rand, metadata={"stub": True}
                )
            else:
                ev = _run_full_ablation(
                    ...
                    label="AMFRS",
                    seed=seed,
                    train_steps=amfrs_train_steps,
                    eval_episodes=eval_episodes,
                    use_stub=False,
                    output_root=os.path.join(work_dir, "table1_amfrs"),
                    n_seeds=n_seeds,
                )
            ...
            results[bucket]["AMFRS"] = ev.to_dict()
            print(format_table1_row(ev))
    else:
        results["no_rand"]["AMFRS"] = {
            "skipped": True,
            "reason": "pass --run-dir with final_candidate.json",
        }
        results["rand"]["AMFRS"] = results["no_rand"]["AMFRS"]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="AMFRS report (baseline paper Table 1 / Table 2)")
    parser.add_argument("--run-dir", type=str, default=None, help="Algorithm 1 output dir")
    parser.add_argument("--output", type=str, default="results/amfrs_report.json")
    ...
    parser.add_argument(
        "--train-amfrs",
        action="store_true",
        help="Actually train AMFRS for Table 1 (slow); default uses stub if --fast",
    )
    ...
    payload: Dict[str, Any] = {
        ...
        "notes": (
            "Faithful AMFRS replication baseline JSON. Raw per-episode records "
    ...
    if do_t1:
        ...
        payload["table1"] = build_table1(
            ...
            include_amfrs_train=args.train_amfrs and not args.fast,
            amfrs_train_steps=full_steps,
            use_stub=use_stub or not args.train_amfrs,
            work_dir=work_dir,
        )
